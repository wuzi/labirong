...
io.on('connection', function (client) {
	...
	client.on('join', function (player) {
		...
		client.emit('addPlayer', { id: client.id, name: player.name, color: player.color, isLocal: true, x: 500, y: 16 });
		game.players.forEach(p => { client.emit('addPlayer', { id: p.id, name: p.name, color: p.color, isLocal: false, x: p.x, y: p.y }); });

		client.broadcast.emit('addPlayer', { id: client.id, name: player.name, color: player.color, isLocal: false, x: 500, y: 16 });

		game.addPlayer({ id: client.id, name: player.name, color: player.color, x: 500, y: 16 });
